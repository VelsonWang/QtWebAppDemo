...
/**
  ...
  <p>
  The logger can keep a configurable number of messages in a ring-buffer.
  If the buffer is disabled, then only messages with severity >= minLevel
  are written out. If the buffer is enabled, then a log message with
  severity >= minLevel flushes the buffer, so the stored messages are
  written out. There is one exception: INFO messages have the highest level,
  but when you use a buffer they are handled like DEBUG messages. So INFO
  messages do not trigger writing the bufferend content.
  <p>
  Each thread has it's own buffer.
  <p>
  Exaple: If you enable the buffer and use minLevel=2, then the application
